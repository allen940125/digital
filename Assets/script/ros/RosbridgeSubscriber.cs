...
public class RosbridgeSubscriber : MonoBehaviour
{
    ...
    private void OnWebSocketMessage(object sender, MessageEventArgs e)
    {
        ...
        {
            // 先解析外層 JSON
            TopicMessageWrapper wrapper = JsonUtility.FromJson<TopicMessageWrapper>(jsonString);

            // 確保 JSON 內有 "topic" 並且等於 "/wheel_speed"
            if (wrapper.topic == topicName)
            {
                float[] data = wrapper.msg.data;
    ...
    private void SetSpeed(ArticulationBody joint, float wheelSpeed = float.NaN)
    {
        ...
        // }
        // drive.targetVelocity = wheelSpeed * 0.10472f;
        drive.targetVelocity = wheelSpeed;
        joint.xDrive = drive;
    }
